...
Contract: `contracts/websocket.yml` (+ ADR-004). Every server→client
message is the `{event, matchId, payload, timestamp}` envelope the
runtime already builds; handlers just route and emit.
"""
...
def build_gateway(registry: MatchRegistry, settings: Settings) -> socketio.AsyncServer:
    ...
    async def _flush(rt: MatchRuntime, disp: Dispatch, actor_sid: str) -> None:
        if disp.error is not None:
            await sio.emit("error", disp.error, to=actor_sid)
            return
        room = _room(rt.match_id)
        for env in disp.public:
            await sio.emit(env["event"], env, room=room)
        if disp.resend_state:
            for sid, uid in list(rt.sessions.items()):
                await sio.emit("game_state", rt.snapshot_envelope(uid), to=sid)
    ...
    async def disconnect(sid: str) -> None:
        ...
        if env is None:
            return
        await sio.emit("player_disconnected", env, room=_room(match_id))
    ...
    async def join_lobby(sid: str, data: dict[str, Any]) -> None:
        ...
        if user_id is None or not isinstance(match_id, str):
            await sio.emit("error", _err(match_id, "bad_request", "missing matchId"), to=sid)
            return
        ...
        for env in rt.join_events(user_id, data.get("sinceActionAt")):
            await sio.emit(env["event"], env, to=sid)
        await sio.emit("player_joined", rt.player_joined_event(user_id), room=_room(match_id))
    ...
    async def player_ready(sid: str, data: dict[str, Any]) -> None:
        ...
        if rt is None or user_id is None:
            return
        await sio.emit(
            "player_joined", rt.player_joined_event(user_id), room=_room(rt.match_id)
        )
